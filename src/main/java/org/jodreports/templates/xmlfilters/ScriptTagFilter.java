...
/**
 * OpenDocument XML entry filter that replaces <tt>script</tt> elements with
 * FreeMarker directives.
 * <p>
 * Scripts can contain FreeMarker directives to be placed at the location of the
 * script itself, or at some enclosing tag. For example a script inside a table
 * cell can contain a <tt>[#list items as item]</tt> directive to be inserted at
 * the enclosing table row so that the entire row will be repeated for each item
 * in the list.
 */
public class ScriptTagFilter extends XmlEntryFilter
{
    ...
    public void doFilter(Document document) throws DocumentTemplateException
    {
        ...
        Nodes scriptNodes = document
                .query("//text:script[translate(@script:language, 'CIJOPRST', 'cijoprst')='jooscript']", XPATH_CONTEXT);
